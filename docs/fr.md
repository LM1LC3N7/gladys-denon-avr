...
- **Volume** — 0-100 %, contrôlable (converti depuis l'échelle interne de l'ampli, -80 dB à +18 dB).
  ...
  pas un bug que cette intégration peut corriger — et ça ne concerne que cette échelle basée sur le
  Telnet, pas la bascule HEOS ci-dessous. Sur un appareil équipé HEOS, si le contrôle Telnet
  classique n'est pas joignable du tout (voir « Support HEOS » ci-dessous — c'est le cas d'une
  enceinte HEOS autonome, pas d'un vrai ampli-tuner), Volume/Volume +/Volume - basculent
  automatiquement sur les commandes HEOS.
- **Muet** — marche/arrêt, contrôlable. Même bascule vers HEOS que Volume ci-dessus quand le
  Telnet n'est pas joignable.
...
- **Diffuser une notification** — alimente l'action de scène native de Gladys **« Parler sur une
  ...
  voir « Support HEOS » ci-dessous). Alimentation, Source et Mode sonore nécessitent encore le
  service Telnet « AVR Control » que ces enceintes n'ont pas — cette intégration n'est donc pas un
  vrai support des enceintes HEOS, juste les fonctionnalités qui ont déjà un équivalent HEOS.
  Déclencher cette action de scène plusieurs fois rapidement remplace ce qui est en train d'être
  dit au lieu de s'accumuler derrière — la file d'attente est vidée avant chaque annonce, confirmé
  nécessaire sur du matériel réel (sans ça, une rafale d'annonces se relisait l'une après l'autre
  au lieu que seule la dernière ne compte).
...
Volume, Volume +/- et Muet fonctionnent dans l'autre sens : les commandes Telnet classiques
restent prioritaires tant que cette connexion est active (c'est le contrôle du volume de la zone
principale, confirmé correct sur un vrai ampli), et ne basculent vers HEOS que si le Telnet n'est
pas joignable du tout — en pratique, une enceinte HEOS autonome sans aucun service « AVR Control ».
Si vos contrôles Volume/Muet ont cessé de répondre après cette mise à jour sur un vrai ampli-tuner,
cela signifierait qu'autre chose ne va pas avec la connexion Telnet (vérifiez les logs de
l'intégration) — ce n'est pas cette bascule qui se déclenche.
